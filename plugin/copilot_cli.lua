vim.api.nvim_create_user_command("CopilotCli", function(opts)
  local commands_menu = require("copilot_cli.commands_menu")

  if #opts.fargs == 0 then
    commands_menu._menu()
  else
    commands_menu._load_command(opts.fargs)
  end
end, {
  desc = "Copilot CLI command interface",
  nargs = "*",
  complete = function(arg_lead, line)
    local cmds = require("copilot_cli.commands_menu").commands
    local parts = vim.split(line:gsub("%s+", " "), " ")

    -- Complete subcommands when typing after main command
    if #parts >= 2 then
      local main_cmd = parts[2]
      if cmds[main_cmd] and cmds[main_cmd].subcommands then
        return vim
          .iter(vim.tbl_keys(cmds[main_cmd].subcommands))
          :filter(function(key)
            return key:find(arg_lead) == 1
          end)
          :totable()
      end
    end

    -- Complete main commands
    return vim
      .iter(vim.tbl_keys(cmds))
      :filter(function(key)
        return key:find(arg_lead) == 1
      end)
      :totable()
  end,
})

-- Track which deprecation warnings have been shown
local deprecated_shown = {}

-- Create a wrapper function for deprecated commands
local function create_deprecated_handler(cmd, replacement, handler)
  return function(opts)
    if not deprecated_shown[cmd] then
      vim.notify(
        ("`%s` is deprecated and will be removed in future versions - use `%s` instead"):format(cmd, replacement),
        vim.log.levels.WARN,
        { title = "copilot-cli" }
      )
      deprecated_shown[cmd] = true
    end
    handler(opts)
  end
end

vim.api.nvim_create_user_command(
  "CopilotHealth",
  create_deprecated_handler("CopilotHealth", "Copilot health", function()
    require("copilot_cli.api").health_check()
  end),
  { desc = "Run copilot-cli health check" }
)

vim.api.nvim_create_user_command(
  "CopilotTerminalToggle",
  create_deprecated_handler("CopilotTerminalToggle", "Copilot toggle", function()
    require("copilot_cli.api").toggle_terminal()
  end),
  { desc = "Toggle Copilot CLI terminal" }
)

vim.api.nvim_create_user_command(
  "CopilotTerminalSend",
  create_deprecated_handler("CopilotTerminalSend", "Copilot send", function(args)
    require("copilot_cli.api").send_to_terminal(args.args)
  end),
  { nargs = "?", range = true, desc = "Send text to Copilot CLI terminal" }
)

vim.api.nvim_create_user_command(
  "CopilotQuickSendCommand",
  create_deprecated_handler("CopilotQuickSendCommand", "Copilot command", function()
    require("copilot_cli.api").open_command_picker()
  end),
  { desc = "Send Copilot slash command to Copilot CLI terminal" }
)

vim.api.nvim_create_user_command(
  "CopilotQuickSendBuffer",
  create_deprecated_handler("CopilotQuickSendBuffer", "Copilot buffer", function()
    require("copilot_cli.api").send_buffer_with_prompt()
  end),
  { desc = "Send buffer to Copilot CLI terminal" }
)

vim.api.nvim_create_user_command(
  "CopilotQuickAddFile",
  create_deprecated_handler("CopilotQuickAddFile", "Copilot add", function()
    require("copilot_cli.api").add_current_file()
  end),
  { desc = "Add current file to Copilot CLI session" }
)
